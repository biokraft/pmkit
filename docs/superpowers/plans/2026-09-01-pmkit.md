...
/// Extended-regex patterns, matched against the command line after runs of
/// whitespace have been collapsed to single spaces. `( [^ ]+)*` skips over any
/// intervening flags and arguments, so `git -C /some/repo push` is caught as
/// well as `git push` -- a literal substring match is trivially bypassed by
/// `git  push` (two spaces) or `git -C dir push`.
///
/// These deliberately over-block rather than under-block: `git commit -m push`
/// being stopped costs the human one "yes", while a missed `git push` costs
/// them a commit on a shared branch they never approved.
        ...
        .map(|(pattern, message)| {
            serde_json::json!({
                ...
                "hooks": [{
                    "type": "command",
                    "command": format!(
                        "raw=$(cat); cmd=$(printf '%s' \"$raw\" | jq -r '.tool_input.command // \"\"' 2>/dev/null); [ -n \"$cmd\" ] || cmd=$raw; printf '%s' \"$cmd\" | tr -s '[:space:]' ' ' | grep -Eq '{pattern}' && {{ echo '{message}' >&2; exit 2; }}; exit 0"
                    )
                }]
            })
        })
...
Three properties of this hook command are load-bearing. Do not simplify any of them:

1. **Exit code 2 is the only code that blocks.** Claude Code's PreToolUse contract treats exit 2 as
   "deny this tool call"; any other non-zero exit is reported as a hook error and does NOT block. A
   hook that exits 1 on a match fails open while the emitted preamble tells the agent its gates are
   machine-enforced -- the worst possible combination.
2. **The jq fallback fails closed.** If `jq` is absent (macOS does not ship it), `cmd` falls back to
   the raw JSON read from stdin, which still contains the command text, so the grep still matches and
   the gate still fires. Dropping the fallback makes every gate silently stop working on a machine
   without `jq`.
3. **Whitespace is collapsed before matching**, which is what lets the `( [^ ]+)*` patterns see
   `git  push` and `git -C dir push`.

Add tests for the bypasses specifically. Each of these must block: `git  push` (two spaces),
`git -C /tmp/repo push`, `echo hi && git push`, `git push --force`, and the jq-absent case. `npm
install` must not block.
...
### Task 13: Cursor gate hooks

**Files:**
- Create: `src/emit/cursor_hooks.rs`
- Modify: `src/emit/cursor.rs`, `src/target.rs`, `src/wizard.rs`
- Modify: `tests/golden/cursor/` (regenerate), `tests/emit_golden.rs`

**Why this task exists:** Cursor's emitted preamble says "The safety gates in this skill are
**machine-enforced** here: a blocked command fails before it runs." pmkit emitted no Cursor hook
configuration whatsoever, so that sentence was false — the one failure mode this product exists to
prevent. Task 6's review checked that the preamble matched `Target::enforces_gates_with_hooks()`,
not that enforcement existed, so the gap survived every earlier review.

**Interfaces:**
- Consumes: `EmitFile`, `FileKind`, the `BLOCKED` pattern table from `emit::claude_code`.
- Produces:
  - `pub fn plan(caps: &Capabilities, dest: &Destination) -> Vec<EmitFile>` additions in `cursor.rs`
  - `impl Target { pub fn gate_config_relpath(self) -> Option<&'static str> }` — `.claude/settings.json`
    for `ClaudeCode`, `.cursor/hooks.json` for `Cursor`, `None` for the three prose-only targets
  - `wizard::next_steps` uses `gate_config_relpath` instead of matching the literal filename
    `settings.json`

**Cursor's hook contract**, from https://cursor.com/docs/hooks — the parts that bind this task:
- Config lives at `.cursor/hooks.json` and carries a top-level `"version": 1`.
- `hooks.beforeShellExecution` is an array of `{ "command": "<shell command>" }`. It runs before the
  agent executes a shell command, and it is the correct place to deny one.
- The payload arrives on stdin as JSON with the command at top level as `.command` — NOT
  `.tool_input.command`, which is Claude Code's shape. Getting this wrong yields a hook that never
  matches and therefore never blocks, while the preamble still promises enforcement.
- **Exit code 2 denies the command**, the same convention as Claude Code's PreToolUse. A
  `{"permission": "deny"}` document on stdout also denies; use the exit code, so one shell idiom
  serves both targets.
- `cursor-agent` (the CLI) sends only `beforeShellExecution` and `afterShellExecution`; the IDE sends
  more. Depending only on `beforeShellExecution` therefore works in both.

- [ ] **Step 1: Write the failing test**

`src/emit/cursor_hooks.rs` test module:
...
mod tests {
    ...
    fn every_hook_denies_with_exit_code_two() {
        ...
        for entry in parsed["hooks"]["beforeShellExecution"].as_array().unwrap() {
            ...
            assert!(cmd.contains("exit 2"), "a hook that does not exit 2 fails open: {cmd}");
        }
    }
}
```

Add to `tests/emit_golden.rs`:
...
fn cursor_emits_a_hooks_file_alongside_its_rules() {
    ...
    assert!(files
        .iter()
        .any(|f| f.path == PathBuf::from("/p/.cursor/hooks.json") && f.kind == FileKind::Config));
}
```

And a behavioural test in the same shape as Task 5's, executing the emitted command through `sh`
with a Cursor-shaped payload. It MUST carry `#[serial(env_path)]` like the Claude Code hook tests.
Each of these must block (exit 2): `git push origin main`, `git  push` (two spaces),
`git -C /tmp/repo push`, `git push --force`, and the jq-absent case. `npm install` must not block.

- [ ] **Step 2: Run tests to verify they fail**

Run: `cargo test --all`
Expected: FAIL — `cursor_hooks` does not exist.

- [ ] **Step 3: Implement**

Make `BLOCKED` visible to the new module (`pub(crate)` in `claude_code.rs`) so the two targets
cannot drift apart on which commands are gated. Then emit `.cursor/hooks.json`, reusing the same
hardened shell idiom as Claude Code with one change — the payload field is `.command`:

```
raw=$(cat); cmd=$(printf '%s' "$raw" | jq -r '.command // ""' 2>/dev/null); [ -n "$cmd" ] || cmd=$raw; printf '%s' "$cmd" | tr -s '[:space:]' ' ' | grep -Eq '<PATTERN>' && { echo '<MESSAGE>' >&2; exit 2; }; exit 0
```

The three load-bearing properties from Task 5 carry over unchanged and for the same reasons: exit 2
is the only code that denies; the `jq` fallback to raw stdin makes a missing `jq` fail closed; and
whitespace is collapsed so the `( [^ ]+)*` patterns see `git  push` and `git -C dir push`.

Add `Target::gate_config_relpath`, and change `wizard::next_steps` to ask it which file to look for
in the outcomes rather than matching the literal string `settings.json`. That is what fixes the
Cursor false-negative: before this task, `gate_installed` looked for a `settings.json` outcome that
Cursor never produced, so a perfectly successful Cursor setup reported "the safety gates are NOT
active here".

- [ ] **Step 4: Regenerate and read the golden files**

Run: `UPDATE_GOLDEN=1 cargo test --test emit_golden`, then read `tests/golden/cursor/.cursor/hooks.json`
and confirm the shell quoting survived the JSON round-trip.

- [ ] **Step 5: Verify by hand**

Extract each emitted hook command and pipe a Cursor-shaped payload (`{"command":"git push"}`) into
`sh -c`, asserting exit 2; then `{"command":"npm install"}`, asserting exit 0. Paste the results
into your report.

- [ ] **Step 6: Commit**

```bash
git add src/emit src/target.rs src/wizard.rs tests
git commit -m "feat: emit Cursor beforeShellExecution hooks so its gates are really enforced"
```
