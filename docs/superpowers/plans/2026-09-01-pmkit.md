...
- [ ] **Step 4: Record the tap formula**

Add to `docs/targets.md` a fenced Ruby block with the formula to paste into
`biokraft/homebrew-tap/Formula/pmkit.rb`, using the release tarball URLs and `sha256` values from
the first published release. Note in the README that `brew upgrade` alone does not refresh the tap —
`brew update && brew upgrade` does.
